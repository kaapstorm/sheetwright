...
def test_recalc_tool_runs_engine_and_caches():
    p = built()
...
@use(built, requires_libreoffice)
def test_snapshot_tool_initializes_then_clean():
    p = built()
...
@use(built)
def test_do_test_tool_passes_when_user_tests_pass():
    p = built()
...
@use(built)
def test_do_test_tool_reports_failure():
    p = built()
    (p / 'tests' / '__init__.py').write_text('')
