...
The on-disk format that sheetwright compiles into an `.xlsx`.

A sheetwright project is a directory of plain text. Two TOML
...
```
my-model/
├── sheetwright.toml         # project config
...
everything under `build/` and `.sheetwright/` is reproducible from
...
rename a sheet, the file rename is mechanical — `sheetwright import`
...
`icon_set`. See `src/sheetwright/model/conditional.py` for the full
...
CSVs in `data/` are loaded into `.sheetwright/bulk.sqlite` on every
...
- `sheetwright.toml`, `workbook.toml`
- `sheets/`
- `data/`
...
- `build/` — regenerated by `sheetwright build`.
- `.sheetwright/` — calc cache, build hash, bulk sqlite, staged
  re-import sessions.

If your team works directly in Excel sometimes, you might choose to
commit `build/<name>.xlsx` so non-sheetwright users can pull from
